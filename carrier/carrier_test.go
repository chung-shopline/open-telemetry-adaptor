...
func TestCarrierNormalFlow(t *testing.T) {
	// set a composite propagator without any sub-propagator is equal to a no-op propagator
	...
	d := NewCarrierFromContext(trace.ContextWithSpanContext(context.Background(), spanContext))
	...
	// recover from JSON
	var c2 Carrier
	err = json.Unmarshal(bz, &c2)
	require.NoError(t, err)
	require.Equal(t, d, c2)

	// recover the trace context
	ctx := c2.PropagateIntoContext(context.Background())
...
func TestCarrierNilFlow(t *testing.T) {
	// default Carrier should work like a no-op propagator without nil pointer dereference
	d := Carrier{}
